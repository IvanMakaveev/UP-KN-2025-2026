...
**Задача 2:** Напишете функция, която приема като параметър два едномерни сортирани целочислени масива с елементи от тип int и връща като резултат трети масив
...
**Задача 4:** Напишете програма, която приема 2 матрици с произволен размер от конзолата от конзолата(2 x 2D arrays). Напишете функция, която сравнява 2-те матрици елемент по елемент и връща true, ако са еднакви(аналогично false ако не са).
...
**Задача 6:** Напишете функция, която приема масив от цели числа и цяло число и елемент на масива. <br />
...
**Задача 7:** Да се напише функция, която приема стринг **text** и символ **s** и връща матрица, за която е заделено точно количество памет, която съдържа всички думи от **text**, разделени с **s**.
...
**Задача 9:** Напишете функции `void insertAt(int*& arr, size_t length, int index, int element)` и `void removeAt(int*& arr, size_t length, int index)`, които добавят / премахват елемент на дадена позиция в динамичен масив. След операцията подаденият масив да е с точно толкова дължина, колкото е нужна.
...
**Задача 10:** Да се дефинира функция `size_t* sortLex(size_t* nums, size_t N)`, която получава като аргументи положителното число N и масив nums, съдържащ N на брой цели неотрицателни числа, и ги сортира във възходящ ред относно лексикографската наредба 
